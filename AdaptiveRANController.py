...
mlflow.set_experiment("AdaptiveRAN")
...
async def main():
    ...
    # Prendre une décision basée sur les prédictions
    ...
    for i, row in enumerate(X_test):
        ...
        if prediction == 0:
            print(f"Ligne {i} : Redirection de l'UE (RSRP={rsrp}, TargetTput={target_tput})")
        elif rsrp < -110:
            print(f"Ligne {i} : Désactivation de l'E2 Node pour améliorer les performances (RSRP={rsrp})")
        elif target_tput > 5:
            print(f"Ligne {i} : Changement de bande passante pour UE avec TargetTput élevé ({target_tput})")
        else:
